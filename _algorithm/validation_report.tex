...
\begin{document}
	...
	{\bodyfont  % Switch to body font for content
		...
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.85\textwidth]{../validation_lift_curve.png}
		...
		The agreement is excellent, with a relative error of $0.61\%$.
		This small discrepancy is attributable to the discrete approximation
		(31 spanwise stations, trapezoidal integration).
		...
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.85\textwidth]{../validation_drag_polar.png}
			\caption{Drag polar for the elliptic wing ($AR=8$).  The computed
			         results follow the theoretical parabola.}
			\label{fig:dragpolar}
		\end{figure}

		The computed drag polar matches the theoretical parabola nearly
		perfectly, confirming that the solver correctly captures the
		induced-drag relation.
		...
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=0.85\textwidth]{../validation_convergence.png}
