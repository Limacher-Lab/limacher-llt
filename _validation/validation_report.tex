...
\newcommand{\validationcommit}{v1.0.0}
\newcommand{\validationversion}{1.0.0}
...
\begin{document}
	...
	{\bodyfont  % Switch to body font for content
		...
		classical analytical results.  The theory is drawn from Anderson's
		Anderson's 	extit{Fundamentals of Aerodynamics}~\cite[Chapter~5]{Anderson2011}.
		The solver is tested on an untwisted elliptic planform of aspect
		ratio $AR = 8$ using thin-airfoil input data
		($c_l = 2\pi\alpha$, $c_d = 0$).

		\noindent	extbf{Validated at version:} 	exttt{\validationcommit}
		...
		\begin{equation}
			...
			\label{eq:liftslope}
		\end{equation}
		where $a_0 = 2\pi$ is the thin-airfoil sectional lift slope when
		sections are assumed to behave according to thin-airfoil theory.
		Equation~\eqref{eq:liftslope} is Anderson's Eq.~(5.69),
		on pg.~443, and substitution of the thin-airfoil lift slope yields
		...
		$+10^\circ$ in $2^\circ$ increments using 31 spanwise stations.
				The results are compared in Table~
ef{tab:liftslope}.
		...
		discrepancy can be attributed to the discrete approximation (31
		spanwise stations, trapezoidal integration), as shown by the
		decreasing error with increasing $N$ in the convergence study
		described below.

		The lift curve is plotted in \Cref{fig:liftcurve}, where the
		computed results from both implementations and the theoretical
		slope are nearly indistinguishable.
		...
		\end{equation}
		This is Anderson's Eq.~(5.61) with the induced drag factor
		$\delta = 0$ for an elliptic planform~\cite[pp.~464--465]{Anderson2011}.

		The drag polar is shown in \Cref{fig:dragpolar}.  Both solvers
		reproduce the theoretical parabola $C_{D_i} = C_L^2 / (\pi AR)$
		nearly perfectly, confirming the correct induced-drag calculation.
		...
		station counts spanning the range 31 to 601.
		...
		against classical theory~\cite{Anderson2011} for an elliptic wing.
		The key results are:
